...
class Backtest:
    ...
    def printMetrics(self):
        ...
        tot_returns = self.returnsAnalyzer.getReturns()
        tot_returns = tot_returns.getValueAbsolute(tot_returns.getMaxLen()-1)
        ...
        print("9. Largest Losing Trade: USDT %.2f" % largest_losing_trade)
        print("10. Largest Winning Trade: USDT %.2f" % largest_winning_trade)
        print("11. Sharpe ratio: %.2f" % self.sharpeRatioAnalyzer.getSharpeRatio(0.05))
        print("12. Sortino ratio: %.2f" % (self.sortino_ratio(self.tradesAnalyzer.getAllReturns())))
        print("13. Total Return: %.2f %%" % ((self.strategy.getResult()-self.broker_cash)/self.broker_cash*100))
